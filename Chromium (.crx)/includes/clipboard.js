...
{
	if(!document.webkitHidden) chrome.runtime.sendMessage( { "data" : "settings" } ); // request settings from background.js
}

function update_gui_all( settings )
{
	if(document.webkitHidden || window.top !== window.self) return;
...
function update_gui(which_part)
{
	...
	{
		document.getElementById("SmartClipboard").innerHTML = content.length === 0 ?
	...
	{
		document.getElementById("SmartClipboard_trash").innerHTML = content.length === 0 ?
	...
	else /* customtext */
	{
		...
		document.getElementById("SmartClipboard_pretext").innerHTML = content.length === 0 ?
