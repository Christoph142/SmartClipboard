...
function quickmenu(){
	...
	if(!window.event[key_for_copy_paste]){ // Ctrl/Cmd released
		if(doc.getElementById("clipboard_tab").style.display === "none"){ // if in slim mode
			hide_clipboard();
			for(i=0; i<doc.getElementsByClassName("clipboard_tab").length; i++) doc.getElementsByClassName("clipboard_tab")[i].style.display = "inline";
		}
		v_pressed = 0;
		ctrl_pressed = 0;
...
function show_clipboard(how)
{
	...
	if(how === "slim")
	{
		...
		for(i=0; i<doc.getElementsByClassName("clipboard_tab").length; i++) doc.getElementsByClassName("clipboard_tab")[i].style.display = "none";
	}
