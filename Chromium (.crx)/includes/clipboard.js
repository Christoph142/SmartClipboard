...
function quickmenu(){
	if(window.event.keyCode === 86){
		v_pressed++;
		if(v_pressed === 1 && doc.getElementById("SmartClipboard_frame").style.display === "inline" && doc.getElementById("clipboard_tab").style.display === "none") 
	...
	if(!window.event[key_for_copy_paste]){ // Ctrl/Cmd released
		if(doc.getElementById("SmartClipboard_frame").className === "slim") hide_clipboard(); // if in slim mode
		v_pressed = 0;
		ctrl_pressed = 0;
...
function show_clipboard(how)
{
	...
	if(how === "slim")
	{
		if(text_saver !== "##_SC_NoInputElement_##"){
			...
			catch(e){	console.log("SmartClipboard error: element_saver no object"); }
		}
	...
	{
		clipboard.className = "";
		try{ document.getElementById("c_SC_0").click(); }catch(e){/* no entry */}
	}
...
function update_gui(which_part)
{
	...
	var content_from_bg = w[which_part];
	
	if(ready !== 1) window.setTimeout(function(){ update_gui(which_part); }, 500);
	else
	{		
		...
		{
			content_from_bg = w.customtext ? w.customtext : [];
		...
		for(i=0; i < content_from_bg.length; i++)
		{
			...
			entry.innerHTML = "<div>\n"+
								"<img src='"+content_from_bg[i].icon+"' />\n"+
								"<span>"+content_from_bg[i].url+"</span>\n"+
							  "</div>\n"+
							  "<div id='"+entry_id+"' class='SmartClipboard_copy_inhibitor' tabindex='"+i+"'>"+content_from_bg[i].txt+"</div>";
